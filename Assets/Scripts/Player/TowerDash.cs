...
public class TowerDash : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (System.Enum.TryParse(AchievementManager.Instance.ReadFromFile("tracker.json", "dashKeybind", true), out KeyCode TparsedKey))
        {
            if(Input.GetKeyDown(TparsedKey) && !isDashing && canDash){   
                BeginDash();
                CompleteAchievementsRunManager.Instance.OnDashUsed();
            }   
        } else {
            if(Input.GetKeyDown(KeyCode.W) && !isDashing && canDash){   
               BeginDash();
               CompleteAchievementsRunManager.Instance.OnDashUsed();
            }
        }
